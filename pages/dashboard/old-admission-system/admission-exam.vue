...
const examConfigs = {
  ...
  du: {
    ...
    negativeMarking: 0.25, passScore: 40, units: ['Ka (Science)', 'Kha (Humanities)', 'Ga (Commerce)', 'Gha (Fine Arts)'],
    description: 'University of Dhaka – All Units'
  },
  cu: {
...
const subjectBreakdown = computed(() => {
  if (!config.value) return []
  const subjs = config.value.subjects
  return Object.keys(subjs).map(subj => {
    ...
    return { subj, label: config.value.subjectLabels[subj], correct, wrong, skipped: total - correct - wrong, total, pct }
  })
})
...
function buildQuestions() {
  ...
  const pool = []
  Object.keys(cfg.subjects).forEach(subj => {
    const needed = cfg.subjects[subj]
...
<template>
  <div class="admission-page">
    ...
    <div v-if="phase === 'setup'" class="setup-phase">
      ...
      <div class="exam-section">
        ...
        <div class="exam-grid varsity-grid">
          ...
          >
            ...
            <div class="exam-card-authority">{{ examConfigs[key].authority }}</div>
            <div class="exam-card-desc">{{ examConfigs[key].description }}</div>
            <!-- Unit selector for DU -->
            <div v-if="examConfigs[key].units && selectedExam === key" class="unit-selector">
      ...
      <!-- CTA Bar -->
      <div v-if="selectedExam" class="cta-bar">
        <div class="cta-bar-left">
          <div class="cta-exam-name">{{ config.fullLabel }}</div>
          <div class="cta-meta">
        ...
        <div class="cta-bar-right">
          <button class="iso-btn iso-btn--ghost" @click="selectedExam = null">Cancel</button>
          <button class="iso-btn iso-btn--fill" @click="showNegativeWarning = true">
            Start Exam →
          </button>
        </div>
      </div>
    ...
    <Teleport to="body">
      <div v-if="showNegativeWarning" class="modal-backdrop" @click.self="showNegativeWarning = false">
        <div class="modal neg-modal">
          ...
          <div class="neg-modal-exam">{{ config?.fullLabel }}</div>
          <div class="neg-rules">
    ...
    <div v-if="phase === 'exam'" class="exam-phase">
      ...
      <div class="exam-topbar">
        <div class="topbar-left">
          <span class="topbar-exam-chip">{{ config.label }}</span>
    ...
    <div v-if="phase === 'results'" class="results-phase">
      ...
      <div class="results-actions">
        ...
        <button class="iso-btn iso-btn--fill" @click="showNegativeWarning = true; selectedExam = selectedExam; phase = 'setup'; nextTick(() => { showNegativeWarning = false; startExam() })">
          Retake Exam
        </button>
      </div>
...
.unit-pills { display: flex; flex-wrap: wrap; gap: 5px; }
.unit-pill {
  font-family: var(--font-mono);
  font-size: 0.58rem;
  padding: 3px 8px;
...
.unit-pill.active { border-color: var(--white); color: var(--white); }
.unit-pill:hover { border-color: var(--border-bright); color: var(--white); }
...
.cta-bar-right { display: flex; gap: 10px; }
...
@media (max-width: 600px) {
  .admission-page { padding: 16px 12px 60px; }
  .medical-grid, .varsity-grid { grid-template-columns: 1fr; }
  .score-hero-pct { font-size: 2.8rem; }
}
</style>
