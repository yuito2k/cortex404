...
watch([pickerSubject, pickerChapter, pickerDiff], () => {
  pickerPage.value = 1
  loadPickerQuestions()
})
        ...
        <div class="qp-modal">
          ...
          <div class="qp-header">
            <div>
              ...
              <span class="qp-sub">{{ pickerSelected.size }} selected</span>
            </div>
          ...
          <div class="qp-list">
            ...
            <div v-else>
              <div v-for="q in pickerQuestions" :key="q.id"
                class="qp-item"
                :class="{ 'qp-item--selected': pickerSelected.has(q.id) }"
                @click="togglePickerQuestion(q.id)">
                ...
                <div class="qp-item-body">
                  <p class="qp-item-text">{{ q.question?.bangla || q.question?.english }}</p>
                  <div class="qp-item-meta">
                    <span>{{ q.subject?.english }}</span>
                    <span>{{ q.chapter?.english }}</span>
                    <span :class="diffCls(q.difficulty_level)">{{ cap(q.difficulty_level) }}</span>
                    <span class="qp-item-id">#{{ q.id }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="qp-pagination">
            <button class="el-pg-btn" :disabled="pickerPage === 1"
              @click="pickerPage--; loadPickerQuestions()">←</button>
            <span class="el-pg-info">Page {{ pickerPage }}</span>
            <button class="el-pg-btn" :disabled="pickerQuestions.length < pickerPerPage"
              @click="pickerPage++; loadPickerQuestions()">→</button>
          </div>

          <div class="qp-footer">
            <span class="qp-selected-count">{{ pickerSelected.size }} questions selected</span>
...
.qp-item-body { flex: 1; display: flex; flex-direction: column; gap: 4px; }
...
.qp-item-meta { display: flex; gap: 8px; flex-wrap: wrap; font-family: var(--font-mono); font-size: 0.46rem; color: var(--gray); }
.qp-item-id { color: var(--dim); }
.qp-pagination { display: flex; align-items: center; justify-content: center; gap: 10px; padding: 10px; border-top: 1px solid var(--border); flex-shrink: 0; }
