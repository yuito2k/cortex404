          ...
          <template v-if="modal.type === 'addQuestion' || modal.type === 'editQuestion'">
            ...
            <div class="modal-form">
              <div class="mf-group">
                ...
                <textarea class="mf-input mf-textarea" :value="modal.data?.text || ''" placeholder="Enter question…" rows="3"></textarea>
              </div>
              <div class="mf-row">
                <div class="mf-group">
                  <label class="mf-label">STREAM</label>
                  <select class="mf-input mf-select">
                    <option v-for="s in ['SSC','HSC','BUET','Medical','DU','BCS','Bank']" :key="s">{{ s }}</option>
                  </select>
                </div>
                <div class="mf-group">
                  <label class="mf-label">DIFFICULTY</label>
                  <select class="mf-input mf-select"><option>Easy</option><option>Medium</option><option>Hard</option></select>
                </div>
              </div>
              <div class="mf-group">
                <label class="mf-label">OPTIONS (A / B / C / D)</label>
                <input class="mf-input" placeholder="Option A" /><input class="mf-input mt4" placeholder="Option B" />
                <input class="mf-input mt4" placeholder="Option C" /><input class="mf-input mt4" placeholder="Option D" />
              </div>
              <div class="mf-row">
                <div class="mf-group">
                  <label class="mf-label">CORRECT ANSWER</label>
                  <select class="mf-input mf-select"><option>A</option><option>B</option><option>C</option><option>D</option></select>
                </div>
                <div class="mf-group">
                  <label class="mf-label">STATUS</label>
                  <select class="mf-input mf-select"><option>Published</option><option>Draft</option></select>
                </div>
              </div>
              <div class="mf-group">
                <label class="mf-label">EXPLANATION (optional)</label>
                <textarea class="mf-input mf-textarea" rows="2" placeholder="Explain the answer…"></textarea>
              </div>
            </div>
            <div class="modal-actions">
              <button class="iso-btn iso-btn--ghost" @click="closeModal()">Cancel</button>
              <button class="iso-btn iso-btn--fill" @click="showToast('Question saved.'); closeModal()">Save Question</button>
            </div>
          </template>
