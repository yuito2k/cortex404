...
// DB exam's questions were AI-generated, but the admin has now switched the
// Questions-tab mode away from AI (e.g. to the manual curated picker) —
// the stored AI question_ids will be dropped/replaced on save.
const modeChangingAwayFromAI = computed(() =>
  !!originalExam.value && originalExam.value.source === 'ai' && mode.value !== 'ai'
)
...
const questionSetChanged = computed(()=>{
  ...
  // A freshly generated (not yet saved) AI batch always counts as a change
  if (mode.value==='ai' && ai.generated.length) return true
  // Otherwise compare against whatever's currently in curatedIds —
  // covers both the manual curated picker and an edited-but-not-regenerated AI set
  const curr = curatedIds.value
...
async function save(newStatus = null) {
  ...
  try {
    ...
    // ── AI mode, but reusing/editing the existing set (no new batch
    //    generated this session) — persist whatever's left in curatedIds,
    //    same as the curated flow above ─────────────────────────────
    if (!form.is_dynamic && mode.value==='ai' && !ai.generated.length) {
      savedIds = curatedIds.value
    }
    ...
    <template v-else>
      ...
      <template v-else-if="step===2">
        ...
        <template v-else>
          ...
          <!-- Existing AI-generated question set (when this exam is ALSO AI in the DB) —
               shown as a picker-style preview, same pattern as the curated list, so the
               admin can drop/re-add individual questions without a full regenerate. -->
          <div v-if="originalExam?.source==='ai' && curatedPreview.length && !ai.generated.length" class="panel">
            <div class="panel-head">
              <span class="panel-title">AI-GENERATED — {{ curatedIds.length }} / {{ form.question_count }} IN SET</span>
              <button class="iso-btn iso-btn--ghost" style="font-size:0.65rem;padding:5px 12px" @click="openPicker">Open Question Picker</button>
            </div>
            <div v-for="(q,i) in curatedPreview" :key="q.id"
              style="display:flex;align-items:flex-start;gap:12px;padding:10px 16px;border-bottom:1px solid var(--border)">
            ...
            <div class="modal-form">
              ...
              <div>
                <button class="iso-btn iso-btn--fill" :disabled="ai.generating" @click="requestRegen">
                  ...
                  {{ ai.generating ? 'Generating…' : (originalExam?.source==='ai' ? '🔁 Wanna reroll it?' : '✨ Generate New Questions') }}
                </button>
              </div>
      ...
      <template v-else-if="step===3">
        <div style="display:grid;grid-template-columns:1fr 340px;gap:20px">
          ...
          <div style="display:flex;flex-direction:column;gap:16px">
            ...
            <!-- Mode-change warning: AI in DB → switched away from AI in the Questions tab -->
            <div v-if="modeChangingAwayFromAI" class="panel"
              style="padding:14px 16px;border-left:3px solid rgba(255,100,100,0.4)">
              <p style="font-family:var(--font-sans);font-size:0.7rem;color:rgba(255,100,100,0.9);margin:0 0 6px;font-weight:600">⚠ Switching away from AI mode</p>
              <p style="font-family:var(--font-sans);font-size:0.68rem;color:var(--gray);margin:0;line-height:1.5">
                This exam's question set is currently <strong style="color:var(--white)">AI-generated</strong>.
                Changing the Questions-tab mode away from AI will erase those stored question IDs for this exam —
                you'll need to pick a new set (via the curated picker) or regenerate before saving.
              </p>
            </div>
