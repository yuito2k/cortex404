...
-- ============================================================
-- SECTION 4: EXAM SESSIONS & RESULTS
-- ============================================================
...
create table if not exists public.exam_results (
  ...
  exam_type        text not null default 'mock',   -- 'mock' | 'practice' | 'admission' | 'hsc_ssc'
...
begin
  ...
  select
    ...
    p.streak,
    -- rank_change: difference from previous rank (0 on first insert)
    coalesce(
...
--   'published' — visible to all students (question-bank.vue, mock-exam.vue)
--   'draft'     — saved but hidden from students; visible in admin panel only
--   'flagged'   — auto-set or manually set when question_reports threshold met
--
-- Profile role / status values:
--   role:   'user' | 'admin'
--   status: 'active' | 'banned' | 'unverified'
--   Admins should access all data via service role key (never expose to client).
--   Use the role field in your Nuxt middleware to guard admin routes.
--
...
--   Use an Edge Function or trigger to do this automatically.
-- ============================================================
